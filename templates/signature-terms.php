<?php
/**
 * Signature Terms printable page
 * URL: /signature-terms/?user_id=ID&terms_type=sms|call|default
 *
 * Priority:
 *  1) Rich editor content (acu_*_terms_html option)
 *  2) External URL iframe (acu_terms_url option — default type only)
...
if ( $terms_type === 'sms' ) {
	$terms_html_raw = (string) get_option( 'acu_sms_terms_html', '' );
	$page_title     = __( 'SMS შეტყობინების პირობები', 'acu' );
	$is_specific    = true;
} elseif ( $terms_type === 'call' ) {
	$terms_html_raw = (string) get_option( 'acu_call_terms_html', '' );
	$page_title     = __( 'სატელეფონო ზარის პირობები', 'acu' );
	$is_specific    = true;
} else {
	$terms_html_raw = (string) get_option( 'acu_terms_html', '' );
	$page_title     = __( 'წესები და პირობები', 'acu' );
}

$terms_url = (string) get_option( 'acu_terms_url', '' );
...
if ( trim( $terms_html_raw ) !== '' ) {
	$terms_html_prepared = ACU_Helpers::prepare_terms_html( $terms_html_raw );
}
?><!doctype html>
...
<title><?php echo esc_html__( 'Terms & Signature', 'acu' ); ?></title>
...
.terms-iframe { width: 100%; min-height: 240mm; border: 1px solid #e5e7eb; border-radius: 6px; background: #fff; }
...
<div class="signature-terms-wrapper page">
	...
	<?php if ( $terms_html_prepared !== '' ) : ?>
		<div class="rules-inner">
			<?php echo $terms_html_prepared; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — already sanitized by prepare_terms_html ?>
		</div>
	<?php elseif ( $is_specific ) : ?>
		<div class="rules-inner">
			<p style="color:#555;"><?php echo esc_html__( 'No content has been configured for this terms type. Please configure it in the Club Member Settings.', 'acu' ); ?></p>
		</div>
	<?php elseif ( $terms_url !== '' ) : ?>
		<iframe class="terms-iframe" src="<?php echo esc_url( $terms_url ); ?>"></iframe>
		<p class="description" style="margin-top:8px;color:#555;"><?php echo esc_html__( 'Loaded from configured URL (no custom editor content provided).', 'acu' ); ?></p>
	<?php else : ?>
		<div class="rules-inner">
			<?php echo wp_kses_post( apply_filters( 'acu_rules_text', $fallback_rules ) ); ?>
		</div>
	<?php endif; ?>
